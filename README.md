...
1. Run `flask --app app run --port=8080 --debug` in the terminal (inside the root folder). This would start the Flask app. Feel free to change the `--port` value, but make sure you also update it in the frontend code. The `--debug` flag ensure the server automatically reloads in case of backend changes.
